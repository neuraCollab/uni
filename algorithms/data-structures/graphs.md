...
- "List neighbors of node u": $O(\deg(u))$ — fast, only touches actual edges.
- "Is there an edge u -> v?": $O(\deg(u))$ worst case (must scan u's neighbor list), unless neighbors are stored in a set (then $O(1)$ average).
...
| Space | $O(V + E)$ | $O(V^2)$ |
| Add edge | $O(1)$ | $O(1)$ |
| Check edge u->v exists | $O(\deg(u))$ ($O(1)$ if neighbor set) | $O(1)$ |
