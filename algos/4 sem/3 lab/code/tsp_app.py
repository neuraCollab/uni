...
class TSPApp:
    ...
    def run_all_tests(self):
        ...
        for test_case in test_cases:
            ...
            # Запуск алгоритмов
            algorithms = {
                "Ближайший сосед": lambda: nearest_neighbor(distances),
                "Имитация отжига": lambda: self.canvas.solve_tsp_simulated_annealing(
                    cauchy=(self.sa_cooling.get()=="cauchy")
                ),
                "Имитация отжига 2": lambda: solve_tsp_simulated_annealing(distances),
            ...
            for name, func in algorithms.items():
                try:
                    ...
                    self.result_text.insert(tk.END, 
                        f"{name}: длина={cost:.2f}, время={elapsed:.4f}с\n")
                except Exception as e:
                    case_results[name] = {
                        "path": "Ошибка",
                        "cost": float('inf'),
