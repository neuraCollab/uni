...
- **PA-II** — a variant with a quadratic penalty term added to the update-size objective, more tolerant of noisy/mislabeled samples (a single bad example influences the weights less abruptly than under PA-I). $C \to \infty$ recovers the original hard-margin Passive-Aggressive rule (no tolerance for margin violations at all) for both variants.
...
- Why does $C \to \infty$ recover the hard-margin update?
