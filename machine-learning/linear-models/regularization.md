...
- **No/low multicollinearity** — features aren't strongly correlated with each other. If they are, $X^\top X$ becomes ill-conditioned (near-singular), so $(X^\top X)^{-1}$ blows up → huge, unstable coefficients that swing wildly with small data changes.
...
**Connection to PCA/SVD:** $X^\top X$ is (up to scaling) the feature covariance matrix once data is centered — the same matrix PCA eigendecomposes. Ridge can be viewed as shrinking coefficients more aggressively along the low-variance principal-component directions (where $X^\top X$ is close to singular) and less along high-variance directions.
...
- `RidgeClassifier` — solves the classification problem as a regression on $\pm 1$-encoded targets (least-squares fit + sign/argmax decision), which makes it noticeably faster to train than `LogisticRegression`, at the cost of not producing calibrated probabilities. `RidgeClassifierCV` adds built-in alpha search.
...
$$\alpha = \frac{1}{n_{\text{samples}} \cdot C}$$
...
$$\alpha \left[ \text{l1\_ratio} \cdot \|w\|_1 + (1 - \text{l1\_ratio}) \cdot 0.5 \|w\|_2^2 \right]$$
...
- What does `alpha` control, and what happens as $\alpha \to 0$ / $\alpha \to \infty$ for Ridge and Lasso?
